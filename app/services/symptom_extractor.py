...
async def extract_symptoms_ner(text: str) -> List[str]:
    """
    ...
    if pipe == "fallback" or not pipe:
        return await _extract_symptoms_llm_fallback(text)
        
    try:
        # Offload CPU-bound transformers work to a thread
...
async def _extract_symptoms_llm_fallback(text: str) -> List[str]:
